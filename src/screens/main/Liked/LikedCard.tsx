...
import { getGuestToken, getToken, setGuestToken, setIsAddCartLoading, setIsAddToCart } from "../../../redux/reducers/authReducer";
...
const LikedCard = ({
  ...
  const onAddToCart = () => {
    ...
    console.log("data?.Book_Id",data?.Book_Id)
    ApiServices.AddToCartItem(params, async ({ isSuccess, response,guestToken }: any) => {
      if (isSuccess) {
      ...
      >
        ...
        <View style={{ ...appStyles.rowjustify, paddingTop: verticalScale(5) }}>
          ...
          <CustomText
            ...
            text={
              Object.keys(bookPriceData || {}).length > 0?  bookPriceData?.originalCurrency == "Rs"
                ? `${bookPriceData?.originalCurrency} ${bookPriceData?.convertedPrice}`
                : `${bookPriceData?.originalCurrency} ${bookPriceData?.originalPrice} = ${bookPriceData?.convertedPrice}`:""
            }
            color={colors.black}
            size={12}
          />
